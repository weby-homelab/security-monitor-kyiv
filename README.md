# 🛡 Монітор Безпеки (Київ)

[![Python](https://img.shields.io/badge/Python-3.12%2B-3776AB?style=flat-square&logo=python&logoColor=white)](https://www.python.org/)
[![Flask](https://img.shields.io/badge/Flask-3.0%2B-000000?style=flat-square&logo=flask&logoColor=white)](https://flask.palletsprojects.com/)
[![Ліцензія](https://img.shields.io/github/license/weby-homelab/security-monitor-kyiv?style=flat-square&label=%D0%9B%D1%96%D1%86%D0%B5%D0%BD%D0%B7%D1%96%D1%8F)](LICENSE)
[![Статус](https://img.shields.io/badge/%D0%A1%D1%82%D0%B0%D1%82%D1%83%D1%81-%D0%90%D0%BA%D1%82%D0%B8%D0%B2%D0%BD%D0%BE-success?style=flat-square)]()

> **Сучасний центр моніторингу безпеки для вашого дому (Київське видання).**

**Монітор Безпеки (Київ)** — це веб-дашборд, що агрегує критично важливу інформацію про безпеку в реальному часі. Розроблений для запуску на домашніх серверах (Raspberry Pi, VPS, Proxmox), він надає миттєвий огляд ситуації за вікном.

---

## 🆕 Примітки до випуску v1.2.1 (PWA та Якість повітря)

Це оновлення фокусується на покращенні моніторингу екології та зручності використання.

### 🌟 Що нового:
1.  **Підтримка PWA (Додаток на телефон):**
    *   Центр безпеки тепер можна встановити як нативний додаток на смартфон.
    *   Додано власну іконку, маніфест та підтримку офлайн-режиму (оболонки).

2.  **Розширений моніторинг повітря:**
    *   **Інтеграція SaveEcoBot:** Додано отримання даних PM1 зі станції №17095 (Борщагівка, Симиренка).
    *   **API Open-Meteo:** Отримання актуальної температури, вологості, PM2.5 та PM10.
    *   **Розумний інтерфейс:** Віджет динамічно показує доступні параметри. Якщо станція не передає PM1, цей показник автоматично приховується.

3.  **Інтерфейс користувача (UI/UX):**
    *   Оновлено дизайн віджета якості повітря (компактні бейджі з параметрами).
    *   Уточнено локацію моніторингу: "Борщагівка (Симиренка)".

---

## ✨ Основні можливості

*   🚨 **Повітряна тривога:**
    *   Миттєвий статус для Києва та області.
    *   Інтегрована жива карта тривог (`alerts.in.ua`).
    *   Візуальна індикація небезпеки (червоний пульсуючий екран).

*   ☁️ **Якість повітря та метеодані:**
    *   Реальні дані через API OpenMeteo.
    *   Індекс якості повітря (US AQI), PM1, PM2.5, PM10.
    *   Актуальна температура та відносна вологість.
    *   Кольорове кодування стану (Відмінне, Помірне, Шкідливе).

*   💡 **Енергопостачання:**
    *   Інтеграція з локальним сервісом [light-monitor-kyiv](https://github.com/weby-homelab/light-monitor-kyiv).
    *   Відображення статусу світла ("Є Світло" / "Немає Світла") в реальному часі.

*   ☢️ **Радіаційний фон:**
    *   Моніторинг рівня гамма-випромінювання (мкЗв/год).

*   🎨 **Сучасний інтерфейс:**
    *   Темна тема (Dark Mode) за замовчуванням.
    *   Адаптивний дизайн для мобільних пристроїв, планшетів та настінних дисплеїв.
    *   Автоматичне оновлення даних без перезавантаження сторінки (AJAX).

---

## 📸 Скріншоти

![Dashboard Screenshot](dashboard_preview.jpg)
*(Приклад інтерфейсу)*

---

## 🛠 Технологічний стек

*   **Бекенд:** Python 3 + Flask (Gunicorn для продакшену).
*   **Фронтенд:** HTML5, CSS3 (Кастомна темна тема), Vanilla JS.
*   **Інтеграції API:**
    *   `ubilling.net.ua` (Тривоги)
    *   `open-meteo.com` (Якість повітря)
    *   Локальний API (Монітор світла)

---

## 🚀 Встановлення та запуск

### 1. Вимоги
*   Linux сервер (Ubuntu/Debian)
*   Python 3.10+
*   Встановлений `light-monitor-kyiv` (опціонально, для статусу світла)

### 2. Встановлення

```bash
# Клонування репозиторію
git clone https://github.com/weby-homelab/security-monitor-kyiv.git
cd security-monitor-kyiv

# Створення віртуального оточення
python3 -m venv venv
source venv/bin/activate

# Встановлення залежностей
pip install -r requirements.txt
```

### 3. Налаштування Systemd (Високе навантаження та Автозапуск)

Для стабільної роботи під навантаженням ми використовуємо Gunicorn з декількома воркерами. Створіть файл `/etc/systemd/system/security-monitor.service`:

```ini
[Unit]
Description=Security Monitor Kyiv (Flask App)
After=network.target

[Service]
User=root
WorkingDirectory=/path/to/security-monitor-kyiv
# Оптимізація: 4 воркери + 2 потоки для обробки запитів без черг
ExecStart=/path/to/security-monitor-kyiv/venv/bin/gunicorn --workers 4 --threads 2 -b 0.0.0.0:5000 app:app
Restart=always

[Install]
WantedBy=multi-user.target
```

Запустіть сервіс:
```bash
systemctl daemon-reload
systemctl enable security-monitor
systemctl start security-monitor
```

### 4. Доступ та Безпека
⚠️ **Важливо:** Не залишайте порт 5000 відкритим у світ (`0.0.0.0`) без захисту.

Рекомендовані способи доступу:
1.  **Cloudflare Tunnel:** Безпечний доступ без відкриття портів (рекомендовано).
2.  **Зворотний проксі Nginx:** Налаштуйте Nginx з SSL перед Gunicorn.
3.  **VPN/Tailscale:** Доступ тільки з внутрішньої мережі.

Приклад доступу: `https://your-secure-domain.com` або `http://localhost:5000` (локально).

---

## 🤝 Участь у розробці

Ми вітаємо будь-які ідеї! Якщо ви хочете додати підтримку нових датчиків або API:
1.  Зробіть форк проєкту.
2.  Створіть гілку (`git checkout -b feature/NewSensor`).
3.  Створіть Pull Request.

---

## 📜 Ліцензія

© 2026 [Weby Homelab](https://github.com/weby-homelab/security-monitor-kyiv) — infrastructure that doesn’t give up  
Made with ❤️ in Kyiv under air raid sirens